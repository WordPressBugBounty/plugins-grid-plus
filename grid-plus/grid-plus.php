...
if (!class_exists('Grid_Plus')) {
    class Grid_Plus
    {
            ...
            if (isset($screen->base)) {
                ...
                if ($screen->base === 'grid-plus_page_grid_plus_setting') {
                    ...
                    $grid_script_data = array(
                        ...
                        'ajax_url' => wp_nonce_url(admin_url('admin-ajax.php'), 'grid_plus_ajax')
                    );
        ...
        {
            $grid_name = isset($atts['name']) ? $atts['name'] : '';
            if (empty($grid_name)) {
                esc_html_e('Missing parameter "name" in shortcode', 'grid-plus');
                return;
            }

            $grid = Grid_Plus_Base::gf_get_grid_by_name($grid_name);
            ...
            if ($layout_type == 'carousel') {
                Grid_Plus_Base::gf_get_template('shortcodes/carousel-shortcode', array(
                    'name' => $grid_name
                ));
            } else {
                Grid_Plus_Base::gf_get_template('shortcodes/grid-shortcode', array(
                    'name' => $grid_name
                ));
            }
            $ret = ob_get_contents();
